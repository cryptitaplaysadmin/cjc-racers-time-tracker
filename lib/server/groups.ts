...
export function normalizeGroupCode(value: unknown) {
  if (typeof value !== 'string') throw new Error('Enter a valid group code.')
  const code = value.trim().replace(/[\s-]/g, '').toUpperCase()
  if (!/^[A-F0-9]{24}$/.test(code)) throw new Error('Enter a valid group code.')
  return code
}
export function cleanAccountName(value: unknown) {
  const name = typeof value === 'string' ? value.trim() : ''
  if (!name || name.length > 80) throw new Error('Account username must be 1–80 characters.')
  return name
}
