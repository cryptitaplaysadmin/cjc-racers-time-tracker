...
test('two group members observe identical alarm deadlines, stopwatch state and shared completion', async () => {
  const f = fixture(), session = f.load('app/api/session/route.ts'), alarms = f.load('app/api/alarms/route.ts')
  const stopwatch = f.load('app/api/group/stopwatch/route.ts'), ack = f.load('app/api/alarms/[id]/ack/route.ts')
  const alice = await session.POST(request('/api/session', { action: 'create', accountName: 'Shared', name: 'Alice' }))
  const { group } = await alice.json()
  const bob = await session.POST(request('/api/session', { action: 'join', joinCode: group.code, name: 'Bob' }))
  f.setCookie(cookieFrom(alice))
  const { alarm } = await (await alarms.POST(request('/api/alarms', { activity: 'farming', cropId: 'melon' }))).json()
  const start = await stopwatch.POST(request('/api/group/stopwatch', { action: 'start', revision: group.revision, label: 'Shared run' }))
  assert.equal(start.status, 200)
  const started = (await start.json()).group
  f.setCookie(cookieFrom(bob))
  const bobView = await (await alarms.GET()).json()
  assert.equal(bobView.alarms[0].scheduledAt, alarm.scheduledAt)
  assert.equal(bobView.group.stopwatch.startedAt, started.stopwatch.startedAt)
  assert.equal((await stopwatch.POST(request('/api/group/stopwatch', { action: 'stop', revision: group.revision }))).status, 409)
  const stoppedResponse = await stopwatch.POST(request('/api/group/stopwatch', { action: 'stop', revision: started.revision }))
  assert.equal(stoppedResponse.status, 200)
  const stopped = (await stoppedResponse.json()).group.stopwatch
  assert.ok(stopped.stoppedAt >= stopped.startedAt)
  assert.equal((await ack.POST(request('/api/alarms/x/ack', { revision: alarm.revision }), { params: Promise.resolve({ id: alarm.id }) })).status, 200)
  f.setCookie(cookieFrom(alice))
  const aliceView = await (await alarms.GET()).json()
  assert.deepEqual(aliceView.group.stopwatch, stopped)
  assert.equal(aliceView.alarms[0].status, 'done')
  const outsider = await session.POST(request('/api/session', { action: 'create', accountName: 'Other', name: 'Other' }))
  f.setCookie(cookieFrom(outsider))
  const otherView = await (await alarms.GET()).json()
  assert.equal(otherView.group.stopwatch, undefined)
  assert.deepEqual(otherView.alarms, [])
})
