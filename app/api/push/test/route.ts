...
export async function POST(request: Request) {
  ...
  try { const session = await requireSession(); const subscription = (await store.subscriptions(session.groupId)).find(item => item.deviceId === session.deviceId); if (!subscription) return NextResponse.json({ error: 'Enable notifications on this device first.' }, { status: 409 }); await sendPush(subscription, JSON.stringify({ title: 'CJC Racers', body: 'Notifications are working on this device.', tag: `cjc-test:${session.deviceId}:${crypto.randomUUID()}`, data: { url: '/' } }), 60); return NextResponse.json({ sent: true }) } catch (error) { return apiError(error, 503) }
}
