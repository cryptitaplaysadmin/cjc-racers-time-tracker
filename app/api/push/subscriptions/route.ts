...
export async function DELETE() { try { const session = await requireSession(); await store.disableSubscription(session.groupId, session.deviceId); return NextResponse.json({ enabled: false }) } catch (error) { return apiError(error, 401) } }
