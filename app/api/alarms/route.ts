...
export async function GET() { try { const session = await requireSession(); const alarms = await store.alarms(session.groupId); return NextResponse.json({ alarms, serverTime: Date.now() }, { headers: { 'Cache-Control': 'no-store' } }) } catch (error) { return apiError(error, 401) } }
