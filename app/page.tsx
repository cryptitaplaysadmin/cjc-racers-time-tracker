...
export default function Page() {
  ...
  return <div className="mx-auto flex min-h-dvh max-w-md flex-col pb-24">
    ...
    <div className="mx-5 mb-5"><AlarmControls onEnableNotifications={clock.requestNotifications} /></div><main className="flex-1">{tab === 'dashboard' ? <div className="space-y-6"><CountdownHero alarm={nextAlarm} now={clock.now} onCreate={() => setTab('alarm')} /><div><div className="mb-3 flex items-center gap-3 px-5"><h2 className="font-display text-sm uppercase tracking-[0.2em] text-muted-foreground">Schedule</h2><span className="h-px flex-1 bg-border" /></div><ScheduleList alarms={upcoming} now={clock.now} onComplete={clock.completeAlarm} onStartTimer={() => setTab('timer')} onDelete={(id) => void clock.deleteAlarm(id).catch(() => clock.refreshSchedule())} /></div></div> : tab === 'alarm' ? <AlarmForm onAdd={async (input) => { await clock.addAlarm(input); setTab('dashboard') }} /> : tab === 'timer' ? <div>{clock.timer && <div className="mx-5 mb-4 rounded-xl border p-4 text-sm">Previous stopwatch: {clock.timer.label}. Started {new Date(clock.timer.startedAt).toLocaleString()}. <button className="underline" onClick={clock.stopTimer}>Save to history</button></div>}<TimerPanel alarms={clock.alarms} now={clock.now} onAdd={clock.addAlarm} onCancel={clock.deleteAlarm} /></div> : <HistoryPanel history={clock.history} onClear={clock.clearHistory} />}</main>
