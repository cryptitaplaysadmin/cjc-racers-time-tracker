...
import { GroupStopwatch } from '@/components/group-stopwatch'
...
export default function Page() {
  ...
  const upcoming = useMemo(() => [...clock.alarms].filter((alarm) => alarm.status !== 'cancelled' && alarm.status !== 'done').sort((a, b) => a.scheduledAt - b.scheduledAt), [clock.alarms, clock.history])
  ...
  return <div className="mx-auto flex min-h-dvh max-w-md flex-col pb-24">
    ...
    <div className="mx-5 mb-5 space-y-2 rounded-xl border border-border p-4 text-sm"><p><span className="font-medium">{clock.name}</span> · shared CJC schedule</p><p className="text-muted-foreground">{clock.pushState === 'enabled' ? 'Global notifications are enabled on this device.' : 'Enable notifications to receive race alerts when this app is in the background.'}</p>{clock.pushError && <p role="alert" className="text-destructive">{clock.pushError}</p>}{clock.pushState === 'enabled' && <div className="flex gap-3"><button className="underline" onClick={() => void clock.testNotifications()}>Send test</button><button className="underline" onClick={() => void clock.disableNotifications()}>Disable here</button></div>}</div>
    <p className="mx-5 mb-3 text-xs text-muted-foreground" role="status">{clock.connectionError ? 'Sync interrupted — retrying' : clock.lastSyncedAt ? 'Auto-sync every 2 seconds while open' : 'Syncing…'}</p><div className="mx-5 mb-5"><AlarmControls onEnableNotifications={clock.requestNotifications} /></div><main className="flex-1">{tab === 'dashboard' ? <div className="space-y-6"><CountdownHero alarm={nextAlarm} now={clock.now} onCreate={() => { setEditing(null); setTab('alarm') }} /><div><div className="mb-3 flex items-center gap-3 px-5"><h2 className="font-display text-sm uppercase tracking-[0.2em] text-muted-foreground">Schedule</h2><span className="h-px flex-1 bg-border" /></div><ScheduleList alarms={upcoming} now={clock.now} onComplete={clock.completeAlarm} onStartTimer={() => setTab('timer')} onDelete={clock.deleteAlarm} canManage={canManage} onEdit={(alarm) => { setEditing(alarm); setTab('alarm') }} /></div></div> : tab === 'alarm' ? <div>{editing && <button className="mx-5 mb-3 underline" onClick={() => setEditing(null)}>Cancel editing</button>}<AlarmForm key={editing?.id ?? 'new'} initial={editing ?? undefined} farmingOnly={editing?.activity === 'farming'} onAdd={async (input) => { if (editing) await clock.editAlarm(editing, input); else await clock.addAlarm(input); setEditing(null); setTab('dashboard') }} /></div> : tab === 'timer' ? <div>{clock.group && <GroupStopwatch group={clock.group} now={clock.now} onChange={clock.updateStopwatch} />}<TimerPanel alarms={upcoming} now={clock.now} onAdd={clock.addAlarm} onCancel={clock.deleteAlarm} canManage={canManage} /></div> : <HistoryPanel history={clock.history} onClear={clock.clearHistory} />}</main>
